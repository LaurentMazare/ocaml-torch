...
let decoder vs =
  ...
  Layer.of_fn (fun xs ->
    ...
    |> Layer.apply l1 |> Tensor.elu
    |> Layer.apply l2 |> Tensor.elu
    |> upsample
    |> Layer.apply l3 |> Tensor.elu
    |> Layer.apply l4 |> Tensor.elu
    |> upsample
    |> Layer.apply l5 |> Tensor.elu
    |> Layer.apply l6 |> Tensor.elu
    |> upsample
    |> Layer.apply l7 |> Tensor.elu
    |> Layer.apply l8 |> Tensor.elu
    |> upsample
    |> Layer.apply l9 |> Tensor.elu
    |> Layer.apply l10 |> Tensor.elu
    |> Layer.apply l11
    |> Tensor.tanh)
...
let encoder vs =
  ...
  let down3 = conv2d vs ~ksize:1 ~padding:0 ~input_dim:(2*num_channel) (3*num_channel) in
  let l7 = conv2d vs ~input_dim:(3*num_channel) (3*num_channel) in
  let l8 = conv2d vs ~input_dim:(3*num_channel) (3*num_channel) in
  let down4 = conv2d vs ~ksize:1 ~padding:0 ~input_dim:(3*num_channel) (4*num_channel) in
  ...
  Layer.of_fn (fun xs ->
    Tensor.to_device xs ~device:(Var_store.device vs)
    |> Layer.apply l0 |> Tensor.elu
    |> Layer.apply l1 |> Tensor.elu
    |> Layer.apply l2 |> Tensor.elu
    |> Layer.apply down1 |> avg_pool2d
    |> Layer.apply l3 |> Tensor.elu
    |> Layer.apply l4 |> Tensor.elu
    |> Layer.apply down2 |> avg_pool2d
    |> Layer.apply l5 |> Tensor.elu
    |> Layer.apply l6 |> Tensor.elu
    |> Layer.apply down3 |> avg_pool2d
    |> Layer.apply l7 |> Tensor.elu
    |> Layer.apply l8 |> Tensor.elu
    |> Layer.apply down4 |> avg_pool2d
    |> Layer.apply l9  |> Tensor.elu
    |> Layer.apply l10 |> Tensor.elu
    |> Tensor.view ~size:[ batch_size; 8 * 8 * 4 * num_channel ]
    |> Layer.apply l11 |> Tensor.elu)
...
let () =
  ...
  Checkpointing.loop ~start_index:1 ~end_index:batches
    ...
    (fun ~index:batch_idx ->
       let learning_rate = learning_rate *. 0.95 **. (Float.of_int batch_idx /. lr_update_step) in
