...
let read_pairs ~input_lang ~output_lang =
  ...
  List.filter_map lines ~f:(fun line ->
      ...
      | [ lhs; rhs ] -> filter_pair (normalize lhs) (normalize rhs)
      | _ -> Printf.failwithf "Line %s is not a tab separated pair" line ())

let create ~input_lang ~output_lang =
  let pairs = read_pairs ~input_lang ~output_lang in
