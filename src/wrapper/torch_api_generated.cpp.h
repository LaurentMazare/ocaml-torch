...
void atg_istft(tensor *out__, tensor self, int64_t n_fft, int64_t hop_length, int64_t win_length, tensor window, int center, int normalized, int onesided, int64_t length) {
  PROTECT(
    auto outputs__ = torch::istft(*self, n_fft, hop_length, win_length, (window ? *window : torch::Tensor()), (bool)center, (bool)normalized, (bool)onesided, length);
    out__[0] = new torch::Tensor(outputs__);
  )
}
