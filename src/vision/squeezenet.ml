...
let squeezenet vs ~version ~num_classes =
  ...
  let features =
    ...
    | `v1_0 ->
      Layer.fold
        [ Layer.conv2d_ vs ~n:(n "conv2d") ~ksize:7 ~stride:2 ~input_dim:3 96
        ; Layer.of_fn Tensor.relu_
        ; Layer.of_fn (Tensor.max_pool2d ~ceil_mode:true ~ksize:(7, 7) ~stride:(2, 2))
        ; fire vs ~n:(n "fire1") 96 16 64 64
        ; fire vs ~n:(n "fire2") 128 16 64 64
        ; fire vs ~n:(n "fire3") 128 32 128 128
        ; Layer.of_fn (Tensor.max_pool2d ~ceil_mode:true ~ksize:(3, 3) ~stride:(2, 2))
        ; fire vs ~n:(n "fire4") 256 32 128 128
        ; fire vs ~n:(n "fire5") 256 48 192 192
        ; fire vs ~n:(n "fire6") 384 48 192 192
        ; fire vs ~n:(n "fire7") 384 64 256 256
        ; Layer.of_fn (Tensor.max_pool2d ~ceil_mode:true ~ksize:(3, 3) ~stride:(2, 2))
        ; fire vs ~n:(n "fire8") 512 64 256 256
        ]
    | `v1_1 ->
      Layer.fold
        [ Layer.conv2d_ vs ~n:(n "conv2d") ~ksize:3 ~stride:2 ~input_dim:3 64
        ; Layer.of_fn Tensor.relu_
        ; Layer.of_fn (Tensor.max_pool2d ~ceil_mode:true ~ksize:(3, 3) ~stride:(2, 2))
        ; fire vs ~n:(n "fire1") 64 16 64 64
        ; fire vs ~n:(n "fire2") 128 16 64 64
        ; Layer.of_fn (Tensor.max_pool2d ~ceil_mode:true ~ksize:(3, 3) ~stride:(2, 2))
        ; fire vs ~n:(n "fire3") 128 32 128 128
        ; fire vs ~n:(n "fire4") 256 32 128 128
  ...
  in
  let final_conv =
    Layer.conv2d_ vs ~n:(n "final_conv") ~ksize:1 ~stride:1 ~input_dim:512 num_classes
  in
  Layer.of_fn_ (fun xs ~is_training ->
    ...
    |> Tensor.relu_
    |> Tensor.adaptive_avg_pool1d ~output_size:[1; 1]
